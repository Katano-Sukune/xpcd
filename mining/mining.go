...
const (
	...
	MinHighPriority = qtumutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr qtumutil.Address) (*qtumutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *qtumutil.Tx, height int32) error {
...
func logSkippedDeps(tx *qtumutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress qtumutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*qtumutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := qtumutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := qtumutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A qtumutil.Block should use saved in the state to avoid
