...
// nodeConfig contains all the args, and data required to launch a xpcd process
...
// arguments returns an array of arguments that be used to launch the xpcd
// process.
...
// command returns the exec.Cmd which will be used to start the xpcd process.
...
// to the xpcd process that is launched via Start().
...
// as the base for the log and data directories for xpcd.
...
// start creates a new xpcd process, and writes its pid in a file reserved for
...
// stop interrupts the running xpcd process process, and waits until it exits
...
// shutdown terminates the running xpcd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := xpcutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
