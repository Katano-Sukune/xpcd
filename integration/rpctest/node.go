...
// nodeConfig contains all the args, and data required to launch a qtumd process
...
// arguments returns an array of arguments that be used to launch the qtumd
// process.
...
// command returns the exec.Cmd which will be used to start the qtumd process.
...
// to the qtumd process that is launched via Start().
...
// as the base for the log and data directories for qtumd.
...
// start creates a new qtumd process, and writes its pid in a file reserved for
...
// stop interrupts the running qtumd process process, and waits until it exits
...
// shutdown terminates the running qtumd process, and cleans up all
