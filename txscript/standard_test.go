...
// newAddressPubKey returns a new xpcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) xpcutil.Address {
	addr, err := xpcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new xpcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) xpcutil.Address {
	addr, err := xpcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new xpcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) xpcutil.Address {
	addr, err := xpcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the xpcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the xpcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := xpcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := xpcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := xpcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := xpcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := xpcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*xpcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*xpcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*xpcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*xpcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*xpcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
