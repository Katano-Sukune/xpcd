...
// newAddressPubKey returns a new qtumutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) qtumutil.Address {
	addr, err := qtumutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new qtumutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) qtumutil.Address {
	addr, err := qtumutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new qtumutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) qtumutil.Address {
	addr, err := qtumutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the qtumutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the qtumutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := qtumutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := qtumutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := qtumutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := qtumutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := qtumutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*qtumutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*qtumutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*qtumutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*qtumutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*qtumutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
