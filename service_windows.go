...
const (
	// svcName is the name of xpcd service.
	svcName = "xpcdsvc"
...
// logServiceStartOfDay logs information about xpcd when the main server has
...
// xpcdService houses the main service handler which handles all service
// updates and launching xpcdMain.
type xpcdService struct{}
...
// long-running xpcdMain (which is the real meat of xpcd), handles service
// change requests, and notifies the service control manager of changes.
func (s *xpcdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start xpcdMain in a separate goroutine so the service can start
...
// installService attempts to install the xpcd service.  Typically this should
...
// removeService attempts to uninstall the xpcd service.  Typically this should
...
// startService attempts to start the xpcd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &xpcdService{})
	if err != nil {
