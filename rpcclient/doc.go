...
Bitcoin RPC server that uses a xpcd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with xpcd (https://github.com/Katano-Sukune/xpcd),
btcwallet (https://github.com/btcsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, xpcd and
...
xpcd or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by xpcd and
...
All notifications provided by xpcd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by xpcd (and
...
Also, it is important to realize that xpcd intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the xpcd RPC server directly, only the RPCs which are related to
...
The following full-blown client examples are in the examples directory:
 ...
 - xpcdwebsockets
   Connects to a xpcd RPC server using TLS-secured websockets, registers for
