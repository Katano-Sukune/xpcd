...
Bitcoin RPC server that uses a qtumd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with qtumd (https://github.com/qtumatomicswap/qtumd),
btcwallet (https://github.com/btcsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, qtumd and
...
qtumd or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by qtumd and
...
All notifications provided by qtumd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by qtumd (and
...
Also, it is important to realize that qtumd intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the qtumd RPC server directly, only the RPCs which are related to
