...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a qtumutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := qtumutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a qtumutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := qtumutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > qtumutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, qtumutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *qtumutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			qtumdLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new qtumd server configured to listen on addr for the
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, qtumdLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *qtumutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
