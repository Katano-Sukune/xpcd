...
const (
	...
	DefaultMinRelayTxFee = qtumutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee qtumutil.Amount) int64 {
	...
	if minFee < 0 || minFee > qtumutil.MaxSatoshi {
		minFee = qtumutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *qtumutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee qtumutil.Amount) bool {
...
func checkTransactionStandard(tx *qtumutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee qtumutil.Amount,
...
func GetTxVirtualSize(tx *qtumutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
