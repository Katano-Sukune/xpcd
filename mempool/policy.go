...
const (
	...
	DefaultMinRelayTxFee = xpcutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee xpcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > xpcutil.MaxSatoshi {
		minFee = xpcutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *xpcutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee xpcutil.Amount) bool {
...
func checkTransactionStandard(tx *xpcutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee xpcutil.Amount,
...
func GetTxVirtualSize(tx *xpcutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
