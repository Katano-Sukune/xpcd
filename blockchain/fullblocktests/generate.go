...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: xpcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := xpcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*xpcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, xpcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount xpcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee xpcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee xpcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee xpcutil.Amount) *wire.MsgTx {
