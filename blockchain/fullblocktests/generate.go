...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: qtumutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := qtumutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*qtumutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, qtumutil.NewTx(tx))
	}
...
func additionalCoinbase(amount qtumutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee qtumutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee qtumutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee qtumutil.Amount) *wire.MsgTx {
