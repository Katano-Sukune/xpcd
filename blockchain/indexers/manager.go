...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block *xpcutil.Block, view *blockchain.UtxoViewpoint) error {
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block *xpcutil.Block, view *blockchain.UtxoViewpoint) error {
...
func makeUtxoView(dbTx database.Tx, block *xpcutil.Block) (*blockchain.UtxoViewpoint, error) {
	...
	for txIdx, tx := range block.Transactions() {
		...
		for _, txIn := range tx.MsgTx().TxIn {
			...
			view.AddTxOuts(xpcutil.NewTx(originTx), 0)
		}
	}
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) ConnectBlock(dbTx database.Tx, block *xpcutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) DisconnectBlock(dbTx database.Tx, block *xpcutil.Block, view *blockchain.UtxoViewpoint) error {
	// Call each of the currently active optional indexes with the block
